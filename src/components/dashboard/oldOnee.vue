    ...
    <div class="statistics_ressults">
      ...
      <div class="statistics_result statistics_result--nonPaid">
        ...
        <div>
          ...
          <p class="supResult">
            <span> خامات :</span> {{ calculateTotalSalesForUnfinishedOrders }}
          </p>
          <p class="supResult supResul--installation">
            <span> مصنعية : </span>
...
export default {
  ...
  computed: {
    ...
    calculateTotalInstallationForAllOrders() {
      ...
      const totalInstallation = this.filteredOrders.reduce((total, order) => {
        if (order.laborPrice) {
          ...
          const orderTotalInstallation = order.products.reduce(
            (subTotal, product) => {
              if (product.productInfo.priceWithLabor) {
                return (
                  subTotal +
                  product.productInfo.priceWithLabor * product.quantity
                );
              } else {
                return subTotal;
              }
      ...
      const totalSales = this.filteredOrders.reduce((total, order) => {
        const orderTotalSales = order.products.reduce((subTotal, product) => {
          const productTotalPrice =
            product.productInfo.priceMaterial * product.quantity;
          const discountAmount =
            productTotalPrice * (product.price_offer / 100);
    ...
    calculateTotalInstallationForUnfinishedOrders() {
      ...
      const unfinishedOrders = this.filteredOrders.filter(
            ...
            <form @submit.prevent="creatNewPurchase">
              ...
              <div class="formbold-input-flex">
                ...
                <div>
                  ...
                  >
                    <option
                      v-for="product in addedPurchase"
                      :value="product.id"
                      :key="product.id"
                    >
                      {{ product.name }}
              ...
              <div class="formbold-input-flex">
                ...
                <div>
                  ...
                  <ValidationProvider
                    name="المنتج"
                    :rules="`${addedPurchase.length == 0 ? 'required' : ''}`"
                    v-slot="{ errors }"
                  >
                    <input
                      readonly
              ...
              <div class="formbold-input-flex">
                <div>
                  <ValidationProvider
                    ...
                    v-slot="{ errors }"
                  >
                    <label for="price_buy" class="formbold-form-label">
                      سعر الشراء
                    </label>
                    ...
                    <p class="info-message">
                      ان قمت بتغير سعر الشراء سيتم تحديثه فى السيستم
                    </p>
                    <span class="error">{{ errors[0] }}</span>
                  </ValidationProvider>
                </div>
                <div>
                  <ValidationProvider
                    ...
                    v-slot="{ errors }"
                  >
                    <label for="price_sell" class="formbold-form-label">
                      سعر البيع
                    </label>
                    ...
                    />
                    <p class="info-message">
                      ان قمت بتغير سعر البيع سيتم تحديثه فى السيستم
                    </p>
                    <span class="error">{{ errors[0] }}</span>
                  </ValidationProvider>
                </div>
              ...
              <div class="formbold-mb-3 cont_add_del_upda">
                <button
                  @click.prevent="updateSingleProduct"
              ...
              <div class="formbold-input-flex">
                <div>
                  <label for="discount_value" class="formbold-form-label">
                    الخصم
                  </label>
                  <input
                    readonly
                ...
                <div>
                  <ValidationProvider name="نوع الخصم" v-slot="{ errors }">
                    <label for="discount_type" class="formbold-form-label">
                      نوع الخصم
                    </label>
                    <input
                      readonly
...
export default {
  ...
  methods: {
    ...
    selectProductForUpdate() {
      const selectedUpdatePro = this.addedPurchase.find(
    ...
    updateSingleProduct() {
      const index = this.addedPurchase.findIndex(
    ...
    updateProductId() {
      ...
      this.price_sell = selectedProductObj
        ? selectedProductObj.priceMaterial
        : "";
    ...
    async addProductsToBranches() {
      ...
      try {
        ...
        for (const product of this.addedPurchase) {
          const existingProduct = branchData.products.find(
    ...
    async creatNewPurchase() {
      ...
      try {
        ...
        sweetalert(
          ...
          "سيتم إعادة توجيهك إلى صفحة التفاصيل.",
          "success"
        );
        this.$router.push({
          name: "DetailsPurchase",
          params: { purchasesId: PurchaseId },
        });
      } catch (error) {
