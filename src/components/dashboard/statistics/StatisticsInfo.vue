  ...
  <div class="statistics">
    <div class="statistics_search">
      ...
      <div class="date-range">
        <div>
          <input type="date" id="to-date" v-model="toDate" placeholder="إلى" />
...
export default {
  ...
  computed: {
    ...
    calculateTotalPurchases() {
      ...
      const FilteredPurchases = this.filterByDateRange(
        this.purchases,
        this.fromDate,
        this.toDate
      );
      ...
      const totalPurchases = FilteredPurchases.reduce((total, purchase) => {
        ...
        const purchaseTotal = purchase.products.reduce((subTotal, product) => {
          ...
          if (product.kindDiscount === "percentage") {
            ...
            discountAmount =
              Number(product.valueDiscountOnBuy) * productQuantity;
          }
        ...
        const returnsForThisPurchase = this.returns.filter(
          (returnItem) => returnItem.purchaseId === purchase.id
        );
        let totalReturns = 0;
            ...
            return (
              ...
              ret.products.reduce((subTotal, product) => {
                ...
                if (product.kindDiscount === "percentage") {
                  ...
                  returnDiscount =
                    Number(product.valueDiscountOnBuy) * productReturnQuantity;
                }

                const productReturnTotalAfterDiscount =
                  productReturnTotal - returnDiscount;
    ...
    calculateTotalPurchasesUnPaid() {
      ...
      const unfinishedOrders = FilteredPurchases.filter(
        (purchases) => purchases.status == "1"
      );
      ...
      const totalPurchases = unfinishedOrders.reduce((total, purchase) => {
        ...
        if (returnsForThisPurchase.length > 0) {
          ...
          // حساب الخصم المطبق على المرتجعات
          const discountOnReturns = this.calculateDiscountOnReturns(
            totalReturns,
            purchase
          );
          totalReturns -= discountOnReturns;
        ...
        // حساب الإجمالي النهائي بعد الخصم، تكلفة الشحن، والمرتجعات
        const finalTotal =
          purchaseTotal - discountAmount + shippingCost - totalReturns;
        return total + finalTotal;
      }, 0);
    ...
    calculateTotalInstallationForAllOrders() {
      const FilteredOrders = this.filterByDateRange(
        this.orders,
        this.fromDate,
        this.toDate
      );
      const getOrderWithInstallationProducts = FilteredOrders?.filter(
        (order) =>
          order.invoiceType === "تركيب" || order.invoiceType === "تركيب وتوريد"
      );

      const totalInstallation = getOrderWithInstallationProducts.reduce(
        (total, order) => {
          ...
          if (order.products) {
            orderTotalInstallation += order.products.reduce(
              (subTotal, product) => {
          ...
          if (order.jokerFields) {
            orderTotalInstallation += order.jokerFields.reduce(
              (subTotal, joker) => {
                if (
                  joker.willBeCalculated &&
                  joker.calculationType === "installation"
                ) {
    ...
    calculateTotalPureInstallationForAllOrders() {
      ...
      const getOrderWithInstalitionProducts = filteredOrders?.filter(
        (order) =>
          order.invoiceType === "تركيب" || order.invoiceType === "تركيب وتوريد"
      );

      const filteredAddTransactions = this.filterByDateRange(
        this.allAddTransactions,
        this.fromDate,
        this.toDate
      );
      ...
      const totalInstallation = getOrderWithInstalitionProducts.reduce(
        (total, order) => {
          let orderTotalInstallation = 0;
                ...
                (subTotal, product) => {
                  if (product.productInfo.priceWithLabor) {
                    const productInstallationPrice = product.editOfInstallation
                      ...
                      : product.productInfo.priceWithLabor;
                    return (
                      subTotal + productInstallationPrice * product.quantity
                    );
                  } else {
                    return subTotal;
                  }
          ...
          if (order.financialTechnicalTransactionId) {
            transaction = this.transactions.find(
              (t) => t.id === order.financialTechnicalTransactionId
            );
          ...
          if (transaction) {
            ...
            const technicianCost = transaction.DailyIndustrial
              ? Number(transaction.numberOfDayes) *
                Number(transaction.DailyIndustrialPrice)
              : Number(transaction.amount) || 0;

            console.log(
              "between transaction ",
              transaction.DailyIndustrial,
              transaction.DailyAssistantIndustrial
            );
            console.log(
              "between transaction valus asis",
              transaction.numberOfDayesAssistant,
              transaction.DailyIndustrialPriceAssistant
            );
            console.log(
              "between transaction valusTotal asia",
              Number(transaction.numberOfDayesAssistant) *
                Number(transaction.DailyIndustrialPriceAssistant)
            );
            ...
            return (
              total +
              (orderTotalInstallation - (technicianCost + assistantCost))
            );
          }
    ...
    calculateTotalSales() {
      ...
      const getOrderWithSellProducts = FilteredOrders?.filter(
        (order) =>
          order.invoiceType === "توريد" || order.invoiceType === "تركيب وتوريد"
      );
      ...
      const totalSales = getOrderWithSellProducts.reduce((total, order) => {
        ...
        if (order.products) {
          orderTotalSales += order.products.reduce((subTotal, product) => {
            ...
            const productTotalPrice =
              Number(productPrice) * Number(product.quantity);
            const discountAmount =
              Number(productTotalPrice) * (Number(product.price_offer) / 100);
        ...
        if (order.jokerFields) {
          orderTotalSales += order.jokerFields.reduce((subTotal, joker) => {
            if (
              joker.willBeCalculated &&
              joker.calculationType === "material"
            ) {
              if (joker.value && joker.quantity) {
                return subTotal + joker.value * joker.quantity;
              } else if (joker.value) {
    ...
    calculateNetProfit() {
      ...
      const netProfit = getOrderWithSellProducts.reduce(
        (totalProfit, order) => {
          ...
          if (order.products) {
            orderNetProfit += order.products.reduce((subTotal, product) => {
              ...
              if (product.productInfo.kindDiscount === "percentage") {
                const discount =
                  productCost *
                  (Number(product.productInfo.valueDiscountOnBuy) / 100);
                productCost -= discount;
              } else if (product.productInfo.kindDiscount === "fixed") {
                const discount =
                  Number(product.productInfo.valueDiscountOnBuy) *
                  productQuantity;
                productCost -= discount;
              }
              ...
              const discountAmount =
                productTotalSellPrice * (Number(product.price_offer) / 100);
              const productSellTotalAfterDiscount =
                productTotalSellPrice - discountAmount;
            ...
            orderNetProfit += order.jokerFields.reduce((subTotal, joker) => {
              if (
                ...
                if (joker.kindDiscount === "percentage") {
                  const discount =
                    jokerCost * (Number(joker.valueDiscountOnBuy) / 100);
                  jokerCost -= discount;
                } else if (joker.kindDiscount === "fixed") {
                  const discount =
                    Number(joker.valueDiscountOnBuy) * jokerQuantity;
                  jokerCost -= discount;
                }
                ...
                const discountAmount =
                  jokerSellTotal * (Number(joker.price_offer || 0) / 100);
                const jokerSellTotalAfterDiscount =
                  jokerSellTotal - discountAmount;
      ...
      const totalUnpaidMaterials = filteredOrders.reduce((total, order) => {
        if (order.financialClientTransactionId) {
          ...
          const transaction = this.transactions.find(
            (t) => t.id === order.financialClientTransactionId
          );
  ...
  methods: {
    ...
    calculateDiscountOnReturns(totalReturns, purchase) {
      ...
      if (purchase.discount_type === "percentage") {
        ...
        discountAmount =
          (purchase.discount_value * totalReturns) / totalPurchases;
      }
