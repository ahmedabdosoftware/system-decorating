  ...
  <div class="dropdown">
    ...
    <div v-if="visible" class="dropdown-content">
      ...
      <a
        v-if="layout === 'DashboardLayout'"
        href="#"
        @click.prevent="delette(order)"
      >
        <span>حذف</span>
        <font-awesome-icon class="icon" :icon="['fas', 'trash']" />
      </a>
      <a
        v-if="isAdmin || shouldDisplayDiscount"
        href="#"
        @click.prevent="Fatora(order.id, false)"
      >
        <span>فاتورة</span>
        <font-awesome-icon class="icon" :icon="['fas', 'file-invoice']" />
      </a>
      <a
        v-if="order.hasCustomInvoice && (isAdmin || isClint)"
        href="#"
        @click.prevent="Fatora(order.id, true)"
      >
        <span> فاتورة (م)</span>
        <font-awesome-icon class="icon" :icon="['fas', 'file-invoice']" />
      </a>
      <a
        v-if="order.financialClientTransactionId && (isAdmin || isClint)"
        href="#"
        @click.prevent="
          financial('TransactionDetails', order.financialClientTransactionId)
        "
      >
        <span>م.العميل </span>
        <font-awesome-icon class="icon" :icon="['fas', 'file-invoice']" />
      ...
      <a
        v-if="order.financialTechnicalTransactionId && (isAdmin || isTechnical)"
        href="#"
        @click.prevent="
          financial('TransactionDetails', order.financialTechnicalTransactionId)
        "
      >
        <span> م.الفنى</span>
        <font-awesome-icon class="icon" :icon="['fas', 'file-invoice']" />
      ...
      <a
        v-if="(order.customerInfo.number && isTechnical) || isAdmin"
        :href="'https://wa.me/2' + order.customerInfo.number"
        target="_blank"
      >
        <span>واتس</span>
  ...
  computed: {
    ...
    shouldDisplayDiscount() {
      if (this.isClint && this.order.hasCustomInvoice) {
        ...
        this.isClint &&
        (this.order.hasCustomInvoice == false ||
          this.order.hasCustomInvoice == "" ||
          this.order.hasCustomInvoice == null)
      ) {
        return true;
      }
      return false;
  ...
  methods: {
    ...
    viewDetails(order) {
      ...
      if (layout === "DashboardLayout") {
        ...
        const profileId = this.$route.params.profileId;
        this.$router.push(
          `/dashboard/profile/${profileId}/DetailsOrder/${order.id}`
        );
      } else if (layout === "profileOutDashboardLayout") {
        const profileId = this.$route.params.profileId;
    ...
    Fatora(orderId, isCustom) {
      ...
      if (layout === "DashboardLayout") {
        ...
        const profileId = this.$route.params.profileId;
        this.$router.push(
          `/dashboard/profile/${profileId}/Fatora/${orderId}/${isCustomPath}`
        );
      } else if (layout === "profileOutDashboardLayout") {
        const profileId = this.$route.params.profileId;
        this.$router.push(
          `/profile/${profileId}/Fatora/${orderId}/${isCustomPath}`
        );
      }
    },
