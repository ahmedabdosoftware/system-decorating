    ...
    <div class="footer">
      <div class="footer_total">
        ...
        <span class="total-label"
          >Total {{ paymentType === "Add" ? "Push" : "Pull" }}:</span
        >
      ...
      <div class="footer_btns">
        ...
        <button
          class="common-btn"
          :disabled="
            (!addHasNextPage && paymentType === 'Add') ||
            (!pullHasNextPage && paymentType === 'Pull')
          "
          @click="
            paymentType === 'Add'
              ? fetchAddTransactions(5, 'next', profileId)
              : fetchPullTransactions(5, 'next', profileId)
          "
        >
          Next
        ...
        <button
          class="common-btn"
          :disabled="
            (!addHasPreviousPage && paymentType === 'Add') ||
            (!pullHasPreviousPage && paymentType === 'Pull')
          "
          @click="
            paymentType === 'Add'
              ? fetchAddTransactions(5, 'previous', profileId)
              : fetchPullTransactions(5, 'previous', profileId)
          "
        >
        ...
        <!-- زر آخر عملية -->
        <button
          class="common-btn spicial-btn"
          @click="
            paymentType === 'add'
              ? fetchAddTransactions(5, null, profileId)
              : fetchPullTransactions(5, null, profileId)
          "
        >
...
export default {
  ...
  methods: {
    ...
    ...mapActions(useRandomTransactionsStore, [
      ...
      "fetchPullTransactions",
    ]),

    // تصفية البيانات بناءً على قيمة hidden
    filterData() {
      if (this.hidden === false) {
        this.filteredData = this.chosseRightData.filter(
