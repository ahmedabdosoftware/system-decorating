...
export default {
  ...
  computed: {
    ...
    isManufacturing() {
      return this.transactionInfo.transactionType === "عميل" && !this.isMaterialActiveOrTechnical;
    },
    ...
    isMaterial() {
     ...
     filteredPayments() {
      if (this.isTechnical) {
        ...
        return this.transactionInfo.payments.filter(payment => payment.type === "material");
      } else if (this.isManufacturing) {
        return this.transactionInfo.payments.filter(payment => payment.type === "industrial");
      }
      return [];
    },
    remainingAmount() {
      ...
      if (this.filteredPayments) {
        totalPayments = this.filteredPayments.reduce((total, payment) => total + parseFloat(payment.amount), 0);
      }
      
      if (this.isTechnical) {
        return (parseFloat(this.transactionInfo.amount || 0) - totalPayments).toFixed(2);
      } else if (this.isManufacturing) {
        return (parseFloat(this.transactionInfo.manufacturingAmount || 0) - totalPayments).toFixed(2);
      } else if (this.isMaterial) {
        return (parseFloat(this.transactionInfo.amount || 0) - totalPayments).toFixed(2);
      }
      return null;
