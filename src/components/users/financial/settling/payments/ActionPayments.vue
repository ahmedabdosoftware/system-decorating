<template>
    <div>
        ...
        <div class="formbold-mb-3 spichial-cont-btns">
          <button class="addProduct-btn spichialAdd-btn" @click="clearTransaction">صفى المعامله</button>
          <button class="addProduct-btn spichialAdd-btn" :class="{ 'disabled-btn': currentTransactionDate=='' || currentAmount=='' }" :disabled="currentTransactionDate=='' || currentAmount==''" @click="addPayment">إضافة دفعة</button>
        </div>
    </div>
  </template>
   ...
   methods: {
    ...
    async addPayment() {
      ...
      if (this.transactionInfo.transactionType === "عميل") {
        newPayment.type = this.isMaterialActiveOrTechnical ? "material" : "industrial";
      }
    ...
    async clearTransaction() {
      ...
      if (this.transactionInfo.transactionType === "فنى") {
        ...
        if (this.isMaterialActiveOrTechnical) {
          ...
          totalAmount = parseFloat(this.transactionInfo.manufacturingAmount) || 0; // إجمالي مصنعية
