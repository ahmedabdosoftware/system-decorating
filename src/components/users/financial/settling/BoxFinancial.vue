    ...
    <div :class="{ 'dark-mode': getDarkMode }">
      ...
      <div class="contBox__toggle">
        <div class="contBox__toggle__chosse">
          <span v-if="isMaterialActiveOrTechnical">{{
            Financial.transactionType === "عميل" ? "خامات" : "فنى"
          }}</span>
          <span v-else>{{
            Financial.transactionType === "عميل" ? "مصنعيه" : "مساعد"
          }}</span>
        </div>
      ...
      <div class="contBox__data">
        ...
        <FinancialDaily
          v-if="
            !isMaterialActiveOrTechnical && Financial.DailyAssistantIndustrial
          "
          :transactionInfo="Financial"
          kind="Assistant"
        />
...
export default {
  ...
  computed: {
    ...
    isTechnicalTransaction() {
      return (
        this.Financial.transactionType === "فنى" &&
        this.isMaterialActiveOrTechnical &&
        this.Financial.payments.length > 0
      );
    },
    isMaterialTransaction() {
      return (
        this.Financial.transactionType === "عميل" &&
        this.isMaterialActiveOrTechnical
      );
    },
    ...
    hasMaterialPayments() {
      return this.Financial.payments.some(
