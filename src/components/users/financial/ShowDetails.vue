  ...
  <div>
    <div class="detailsContent">
      <div class="detailsContent__title">
        <p>
          تفاصيل المعاملة: (رقم الطلب:
          {{ transactionInfo.numberOfOrder || "غير محدد" }})
        </p>
      </div>
      <div class="detailsContent__Info">
        ...
        <div
          v-if="isClientTransaction && materialAccountAmount > 0"
          class="detailsContent__Info__general"
        >
          <p>المستحق كخامات : {{ materialAccountAmount }}</p>
          <p>باقى الخامات: {{ remainingMaterialAmount }}</p>
        </div>
        <div
          v-if="isClientTransaction && manufacturingAccountAmount > 0"
          class="detailsContent__Info__general"
        >
        ...
        <div
          v-if="isTechnicalTransaction && transactionInfo.DailyIndustrial"
          class="detailsContent__Info__general"
        >
          <p>عدد الأيام: {{ transactionInfo.numberOfDayes }}</p>
          <p>
            عدد الأيام المدفوعة: {{ transactionInfo.numberOfDayesPaid || 0 }}
          </p>
        </div>
        <div
          v-if="isTechnicalTransaction && transactionInfo.amount"
          class="detailsContent__Info__general"
        >
        ...
        <div
          v-if="
            isTechnicalTransaction && transactionInfo.DailyAssistantIndustrial
          "
          class="detailsContent__Info__general"
        >
          <p>اسم المساعد : {{ transactionInfo.nameOfAssistant }}</p>
          <p>
            عدد الأيام للمساعد: {{ transactionInfo.numberOfDayesAssistant }}
          </p>
          <p>
            الأيام المدفوعة للمساعد:
            {{ transactionInfo.numberOfDayesPaidAssistant || 0 }}
          </p>
        </div>
      ...
      <div
        v-if="transactionInfo.payments.length > 0"
        class="detailsContent__listProducts"
      >
        <div>
          <p>:الدفعات</p>
        </div>
          ...
          <table class="table">
            <thead>
              <tr>
                <th>المبلغ</th>
                <th>تاريخ الدفع</th>
                <th v-if="transactionInfo.transactionType === 'عميل'">
                  نوع المعامله
                </th>
              </tr>
            </thead>
            <tbody v-if="transactionInfo.payments">
              <tr
                v-for="(payment, index) in transactionInfo.payments"
                :key="index"
              >
                <td>{{ payment.amount }}</td>
                <td>{{ payment.transactionDate }}</td>
                <td v-if="transactionInfo.transactionType === 'عميل'">
                  {{ payment.type == "material" ? "خامات" : "مصنعيه" }}
                </td>
              </tr>
            </tbody>
          </table>
  ...
  computed: {
    ...
    remainingManufacturingAmount() {
      if (this.transactionInfo.payments) {
        const totalManufacturingPayments = this.transactionInfo.payments
          ...
          .reduce((total, payment) => total + parseFloat(payment.amount), 0);
        return (
          this.manufacturingAccountAmount - totalManufacturingPayments
        ).toFixed(2);
      }
      return null;
    },
    ...
    remainingAmount() {
      if (this.transactionInfo.amount && this.transactionInfo.payments) {
        const totalPayments = this.transactionInfo.payments.reduce(
          (total, payment) => total + parseFloat(payment.amount),
          0
        );
        return (
          parseFloat(this.transactionInfo.amount) - totalPayments
        ).toFixed(2);
      }
      return null;
    },
