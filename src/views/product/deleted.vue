  ...
  <ValidationObserver ref="observer" v-slot="{ invalid }">
    <div class="add-product">
      ...
      <div class="add-product__title">
        <p :class="{ 'add-product__title--dark-mode': getDarkMode }">create product</p>
      </div>

      <div :class="{ 'add-product_content--dark-mode': getDarkMode }" class="add-product_content">
        <form class="add-product__form" @submit.prevent="creatNewProduct">
          <div class="add-product_field add-product_field--name">
            <label :class="{ 'add-product__label--dark-mode': getDarkMode }">product title</label>
            <ValidationProvider
              name="Product Title"
              rules="required"
              v-slot="{ errors }"
          ...
          <div class="add-product_field add-product_field--description">
            <label :class="{ 'add-product__label--dark-mode': getDarkMode }">description</label>
            <ValidationProvider
              name="Description"
              rules="required"
              v-slot="{ errors }"
            ...
            <p class="add-product__note">you will be able to edit it later.</p>
          </div>
          
          <div class="add-product__image-container">
            <ValidationProvider
              name="Image"
            ...
            <div class="add-product_upload-container add-product_upload-container--image">
              <img id="up" class="add-product__upload" v-bind:src="imageUrl[0]" />
            </div>
            <div class="add-product_upload-container add-product_upload-container--image">
              <img id="up" class="add-product__upload" v-bind:src="imageUrl[1]" />
            </div>
            <div class="add-product_upload-container add-product_upload-container--image">
              <img id="up" class="add-product__upload" v-bind:src="imageUrl[2]" />
            </div>
            <div v-on:click="click()" id="btn" class="add-product_upload-container add-product_upload-container--upload">
              <img
              ...
              />
              <span class="add-product__upload-label">upload</span>
            </div>
          </div>
          <div class="add-product_field add-product_field--tags">
            <label>tags</label>
            <input v-model="tags" type="text" placeholder="enter tags" />
          </div>
          <div class="add-product_field add-product_field--category">
            <label :class="{ 'add-product__label--dark-mode': getDarkMode }">category</label>
          ...
          <div class="add-product__price-container">
            <div class="add-product__price">
              <div>
                <label :class="{ 'add-product__label--dark-mode': getDarkMode }">price Material</label>
                <ValidationProvider
                  name="Price Material"
              ...
              <div>
                <label :class="{ 'add-product__label--dark-mode': getDarkMode }">price With Labor</label>
                <ValidationProvider
            ...
            <div class="add-product__price offer">
              <label :class="{ 'add-product__label--dark-mode': getDarkMode }">offer price (optionally)</label>
              <ValidationProvider
                name="Offer Price"
          ...
          <div class="add-product__options">
            <div class="add-product__price-options">
              <label>
                <input type="checkbox" v-model="showLaborPrice" />
                Show Labor Price on Site
              </label>
            </div>
            <div class="add-product__display-options">
              <label>
          ...
          <div class="add-product__submit-container">
            <button class="add-product__submit" :disabled="invalid" @click="creatNewProduct">submit item</button>
          </div>
        </form>
      </div>
      <div id="loader" class="add-product__loader">
...
.add-product_content {
  ...
  background-color: rgb(110, 71, 22);
...
.add-product__form {
  ...
  .add-product_field--description {
    height: 100px;

  }
    .add-product_field--description input {
    height: 45px;
    background-color: red;
  }

  .add-product__submit-container {
    display: flex;
    align-items: center;
    justify-content: flex-start;
    width: 100%;
...
.add-product__image-container {
  display: flex;
  justify-content: space-evenly;
  align-items: center;
  background-color: fuchsia;
  height:150px;
  .add-product_upload-container {
...
.add-product__price-container{
  ...
  > div:first-child {
      ...
      >div{
        background-color: rgb(134, 97, 116);
        width: 50%;
        display: flex;
        flex-direction: column;
        padding-left: 20px;
      ...
      <form @submit.prevent="submit">
        <div class="formbold-input-flex">
          <div>
            ...
            <ValidationProvider name="ربط بطلب" rules="required" v-slot="{ errors }">
          ...
          <div>
            ...
            <ValidationProvider name="القيمة المستحقة" rules="required|numeric|min_value:1" v-slot="{ errors }">
        ...
        <div class="formbold-input-flex">
          <div>
            ...
            <ValidationProvider name="المبالغ المضافة الآن" rules="required" v-slot="{ errors }">
        ...
        <div class="formbold-input-flex">
          <div>
            ...
            <ValidationProvider name="المبلغ" rules="required|numeric|min_value:1" v-slot="{ errors }">
          ...
          <div>
            ...
            <ValidationProvider name="تاريخ استلام الدفعة" rules="required|date_format:YYYY-MM-DD" v-slot="{ errors }">
        ...
        <div class="formbold-mb-3">
          ...
          <ValidationProvider name="تاريخ للمعاملة" rules="required_if:!selectedOrder|date_format:YYYY-MM-DD" v-slot="{ errors }">
        ...
        <div class="formbold-mb-3">
          ...
          <ValidationProvider name="حدد مكان للمعاملة" rules="required_if:!selectedOrder" v-slot="{ errors }">
            <input type="text" id="address" placeholder="العنوان" class="formbold-form-input formbold-mb-3" v-model="address">
            <span>{{ errors[0] }}</span>
          </ValidationProvider>
        </div>

        <div class="formbold-mb-3">
          <label for="message" class="formbold-form-label">ملاحظات</label>
          <textarea rows="6" id="message" class="formbold-form-input" v-model="notes"></textarea>
        </div>

        <button class="formbold-btn" :disabled="invalid">حفظ والاطلاع على التفاصيل</button>
      </form>
     </ValidationObserver>
    </div>
  </div>
...
extend('required_if', {
...required_if,
message: 'هذا الحقل مطلوب او حدد طلب  لاستخارج التاريخ منه.',
});
...
//debug color => will delete 
$test_color:rgb(92, 89, 89);
$header_color: #9f9bec;
$main_color: rgb(185, 173, 247);
$buttom_color: #6a64f1;
$buttom_font: 19px;
...
.addProduct-btn,.updateProduct-btn{
  @extend %btn-style;
  width: 200px;
  height: 40px;  
}
...
.deleteProduct-btn,.updateProduct-btn{
display:none;
}
.addProduct-btn_hidde{
display:none;
}
.deleteProduct-btn_show,.updateProduct-btn_show{
display:block;
}
...
<ValidationProvider name="تاريخ استلام الدفع" :rules="{ payment_required: { payments: payments }, date_format: 'YYYY-MM-DD' }" v-slot="{ errors }">
  <ValidationProvider name="المبغ" :rules="{ payment_required: { payments: payments } }" v-slot="{ errors }">                
...
<template>
  ...
  <div class="category">
    ...
    <div :class="{ 'dark-mode-box': getDarkMode }" class="allContent">
      <div v-if="isLoading">
        <SkeletonBox v-for="n in 6" :key="n" />
      </div>
      <div v-else>
        <div v-for="category in filteredCategories" :key="category.id">
          <BoxCategory :category="category"></BoxCategory>
        </div>
...
import BoxCatogery from "@/components/categories/BoxCatogery.vue";
 ...
 // Skeleton Box
 import SkeletonBox from '@/shared/components/loading/skeletonLoader/SkeletonBox.vue';
