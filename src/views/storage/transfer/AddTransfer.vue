        ...
        <div class="formbold-form-wrapper">
          ...
          <ValidationObserver ref="observer" v-slot="{ invalid }">
            <form @submit.prevent="creatNewTransferBranch">
              <div class="formbold-input-flex">
                <div>
                  <ValidationProvider
                    ...
                    v-slot="{ errors }"
                  >
                    <label for="selectedBranchTo" class="formbold-form-label">
                      *الفرع *الى</label
                    >
                    <select
                ...
                <div>
                  <ValidationProvider
                    ...
                    v-slot="{ errors }"
                  >
                    <label for="selectedBranchFrom" class="formbold-form-label">
                      *الفرع *من</label
                    >
                    <select
                      class="formbold-form-input"
              ...
              <div class="formbold-input-flex">
                <div>
                  <label for="addedInBranch" class="formbold-form-label"
                    >المنتجات المحوله
                  </label>
                  <select
                    class="formbold-form-input"
                  ...
                  >
                    <option
                      v-for="product in transferdInBranch"
                      :value="product.id"
                      :key="product.id"
                    >
                      {{ product.name }}
                ...
                <div>
                  ...
                  >
                    <option
                      v-for="product in branchTransferFrom.products"
                      :value="product.id"
                      :key="product.id"
                    >
                      {{ product.name }}
              ...
              <div class="formbold-input-flex">
                ...
                <div>
                  ...
                  <ValidationProvider
                    name="المنتج"
                    :rules="`${
                      transferdInBranch.length == 0 ? 'required' : ''
                    }`"
                    v-slot="{ errors }"
                  >
                    <input
              ...
              <div class="formbold-mb-3 cont_add_del_upda">
                <button
                  ...
                  :disabled="!selectedProductName || !quantity"
...
export default {
  ...
  computed: {
    ...
    branchTransferFrom() {
      const branch = this.branches.find(
        (branch) => branch.id === this.selectedBranchFrom
      );
      if (branch) {
    ...
    branchTransferTo() {
      const branch = this.branches.find(
        (branch) => branch.id === this.selectedBranchTo
      );
      if (branch) {
  ...
  methods: {
    ...
    selectProductForshowData() {
      const selectedUpdatePro = this.branchTransferFrom.products.find(
        (product) => product.id === this.selectProductForShowDataProp
      );
    ...
    selectProductForUpdate() {
      const selectedUpdatePro = this.transferdInBranch.find(
        (product) => product.id === this.selectProductForShowDataProp
      );
      if (selectedUpdatePro) {
        this.selectedProductName = selectedUpdatePro.name;
    ...
    updateProduct() {
      const index = this.transferdInBranch.findIndex(
        (product) => product.id === this.productId
      );
      if (index !== -1) {
        this.transferdInBranch[index].name = this.selectedProductName;
        this.transferdInBranch[index].transferedQuantity =
          this.transferedQuantity;
        this.clearProductForm();
      }
    },
    ...
    async creatNewTransferBranch() {
      ...
      try {
        if (this.transferdInBranch.length === 0) {
          sweetalert(
            "لا يمكن تحويل المخزون بدون منتجات",
            "يرجى إضافة منتجات قبل التحويل .",
            "error"
          );
          ...
          sweetalert(
            ...
            "يرجى تحديد فرعين مختلفين  قبل التحويل .",
            "error"
          );
          this.isLoading = false;
          return;
        ...
        this.transferdInBranch.forEach((product) => {
          const productInBranchFrom = this.branchTransferFrom.products.find(
            (p) => p.id === product.id
          );
        ...
        this.transferdInBranch.forEach((product) => {
          const productInBranchTo = this.branchTransferTo.products.find(
            (p) => p.id === product.id
          );
          if (productInBranchTo) {
            productInBranchTo.quantity =
              Number(productInBranchTo.quantity) +
              Number(product.transferedQuantity);
          } else {
