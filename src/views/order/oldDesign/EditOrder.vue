          ...
          <ValidationObserver ref="observer" v-slot="{ invalid }">
            <form @submit.prevent="handelUpdateOrder">
              <div class="formbold-input-flex">
                <div>
                  <label for="customer" class="formbold-form-label aline-end"
                    >اختر العميل</label
                  >
                  <input
                    list="customerList"
                  ...
                  <p v-if="unregisteredCustomerMessage" class="warning-message">
                    {{ unregisteredCustomerMessage }}
                  </p>
                </div>

                <div>
                  <label for="technical" class="formbold-form-label aline-end">
                    :اختر الفنى
                  </label>
                  <input
                    list="technicalList"
                    class="formbold-form-input aline-end"
                    placeholder="ابحث عن فنى"
                  ...
                  <p
                    v-if="unregisteredTechnicalMessage"
              ...
              <div class="formbold-input-flex">
                <div>
                  <label for="shipping" class="formbold-form-label aline-end">
                    الشحن
                  </label>
                  <input
                    type="number"
                ...
                <div>
                  <ValidationProvider
                    ...
                    v-slot="{ errors }"
                  >
                    <label class="formbold-form-label aline-end"
                      >نوع الفاتورة</label
                    >
                    <select
              ...
              <div class="formbold-input-flex">
                <div>
                  <ValidationProvider
                    ...
                    v-slot="{ errors }"
                  >
                    <label for="status" class="formbold-form-label aline-end"
                      >حالة الطلب
                    </label>
                    <select
                      id="status"
                ...
                <div>
                  <label for="addedOrders" class="formbold-form-label aline-end"
                    >المنتجات المضافة الآن</label
                  >
                  <select
                    class="formbold-form-input"
              ...
              <div class="formbold-input-flex">
                <div>
                  <ValidationProvider
                    ...
                    v-slot="{ errors }"
                  >
                    <label for="quantity" class="formbold-form-label aline-end">
                      الكمية
                    </label>
                ...
                <div>
                  <label for="product" class="formbold-form-label aline-end">
                    المنتج
                  </label>
                  <ValidationProvider
                    name="المنتج"
                    :rules="`${addedOrders.length == 0 ? 'required' : ''}`"
                    v-slot="{ errors }"
                  >
                    <input
                      list="propList"
                    ...
                    <datalist id="propList">
                      <option
                        v-for="product in myAllProducts"
                        :key="product.id"
                        :value="product.name"
                      ></option>
                    </datalist>
              ...
              <div class="formbold-input-flex">
                ...
                <div>
                  <label for="priceOffer" class="formbold-form-label aline-end">
                    الخصم
                  </label>
                  <input
                    type="number"
              ...
              <div class="formbold-input-flex" v-if="pullFromBranch">
                <div>
                  <ValidationProvider name="الكمية المتاحة" v-slot="{ errors }">
                    <label
                      for="quantityInBranch"
                      class="formbold-form-label aline-end"
                    >
                      الكمية المتاحة
                    </label>
                    <input
                      ...
                      readonly
                    />
                    <p v-if="isEditingProduct" class="info-message">
                      بالاضافة لكمية السحب السابقة على نفس الفرع
                    </p>
                    <p
                      v-if="aboutQuantityInBranchMessage"
                      class="warning-message"
                    >
                ...
                <div>
                  <label for="branch" class="formbold-form-label aline-end">
                    الفروع المتوفر بها
                  </label>
                  <ValidationProvider name="الفروع" v-slot="{ errors }">
                    <input
                      list="List"
                      class="formbold-form-input aline-end"
                      placeholder=" ابحث فى الفروع"
                      v-model="selectedBranch"
                      @input="getProductFromBranch"
                    />
                    <datalist id="List">
                      <option
                        v-for="branch in getAllBranchesHasProduct"
                        :key="branch.id"
                        :value="branch.nameBranch"
                      ></option>
                    </datalist>
                    <p
                      v-if="getAllBranchesHasProduct.length == 0"
                      class="warning-message"
                    >
                      غير متوفر فى اى فرع
                    </p>
              ...
              <p
                v-if="pullFromBranch && hasPreviousPullOperation"
                class="aline-end m-btn"
              >
                : آخرعملية سحب
              </p>
              ...
              <div class="formbold-input-flex">
                <div>
                  ...
                  >
                    ...
                    <p class="info-message">
                      اتركه فارغا ليتم انشاء رقم الطلب اوتوماتيكلى
                    </p>
                    <span class="error">{{ errors[0] }}</span>
                  </ValidationProvider>
                </div>
                ...
                <div>
                  <label for="laborPrice" class="formbold-form-label aline-end"
                    >حدد المصنعية يدويا
                  </label>
                  <input
                    type="number"
                  ...
                  />
                  <p class="info-message">
                    فى حالة لا يوجد منتج يتم حساب المصنعية من خلاله او اذا كنت
                    تفضل حساب المصنعية يدويا
                  </p>
                </div>
              </div>
              ...
              <div class="formbold-mb-3">
                <ValidationProvider
                  name="تاريخ بدا العمل"
                  rules="date_format:YYYY-MM-DD"
                  v-slot="{ errors }"
                >
                  <label for="dob" class="formbold-form-label aline-end">
                    تاريخ بدا العمل
                  </label>
                  <input
                    type="date"
              ...
              <div class="formbold-mb-3">
                <label for="address" class="formbold-form-label aline-end">
                  مكان التركيب
                </label>
                <input
                  type="text"
              ...
              <button
                ...
                :disabled="invalid"
              >
                حفظ و إنشاء الفاتورة
              </button>
              <button
                @click.prevent="createAndCustomizeInvoice"
                class="formbold-btn"
                :class="{ 'disabled-btn': invalid }"
                :disabled="invalid"
              >
...
export default {
  ...
  methods: {
    ...
    selectProductForUpdate() {
      const selectedUpdatePro = this.addedOrders.find(
        (product) => product.id === this.selectProductForUpdateProp
      );
      if (selectedUpdatePro) {
        ...
        if (
          (selectedUpdatePro.branchId && selectedUpdatePro.isPulled) ||
          selectedUpdatePro.ancientBranchId
        ) {
    ...
    showLastPullOpertion(selectedProduct) {
      ...
      if (selectedProduct.ancientBranchId) {
        ...
        lastPulledBranch = this.getBranchNameById(
          selectedProduct.ancientBranchId
        );
      ...
      console.log(
        `Last Pulled Branch: ${lastPulledBranch}, Last Pulled Quantity: ${lastPulledQuantity}`
      );
    },
    ...
    updateProduct() {
      ...
      if (index !== -1) {
        ...
        if (this.pullFromBranch && this.addedOrders[index].isPulled) {
          ...
          if (this.BranchId) {
            ...
            if (this.addedOrders[index].ancientBranchId === this.BranchId) {
              ...
              if (branchQuantity <= 0) {
                console.log(
                  "Error: لا يمكن السحب من الفرع الجديد لأن الكمية المتاحة أقل من أو تساوي الصفر"
                );
                sweetalert({
          ...
          console.log(
            `Old branchId: ${ancientBranchId}, Old pullQuantityNeeded: ${ancientQuantityNeeded}`
          );
        }
        ...
        if (
          this.pullFromBranch &&
          Number(this.quantityInBranch) > 0 &&
          !("isPulled" in this.addedOrders[index])
        ) {
          let pullQuantityNeeded = Number(this.quantity);
          let branchQuantity = Number(this.quantityInBranch);
        ...
        console.log(
          ...
          this.BranchId,
          this.pullFromBranch,
          this.addedOrders[index].isPulled
        );
        ...
        if (this.pullFromBranch == false && this.addedOrders[index].isPulled) {
          ...
          if (
            ...
            var ancientQuantityNeededd =
              this.addedOrders[index].pullQuantityNeeded;

            if (this.addedOrders[index].pullStatus === "less") {
              ancientQuantityNeededd = Number(
                this.addedOrders[index].avilabelQuantityInBranch
              );
            }
    ...
    deleteProduct() {
      ...
      if (index !== -1) {
        ...
        if (
          !this.addedOrders[index].branchId &&
          !this.addedOrders[index].ancientBranchId
        ) {
        ...
        if (
          ...
          console.log(
            "Product has a pull operation but just added, simply delete"
          );
          this.addedOrders.splice(index, 1);
          this.clearProductForm();
          return;
        ...
        if (this.addedOrders[index].isPulled) {
          console.log(
            "Product has a pull operation from before, adjusting quantities"
          );
    ...
    updateProductId() {
      ...
      const selectedProductObj = this.myAllProducts.find(
    ...
    getProductFromBranch() {
      const getBranch = this.branches.find(
        (branch) => branch.nameBranch === this.selectedBranch
      );
      let product;
      if (getBranch) {
        this.BranchId = getBranch.id;
        product = getBranch.products.find(
          (product) => product.id === this.productId
        );
      }
    ...
    quantityMessage() {
      if (this.BranchId) {
        ...
        if (Number(this.quantityInBranch) < Number(this.quantity)) {
          this.aboutQuantityInBranchMessage =
            "سيتم سحب الكمية المتوفرة والباقى يمكنك شراءه"; // الكمية أقل مما يريد
        } else {
    ...
    updateCustomerId() {
      ...
      if (selectedCustomerObj) {
        ...
        this.unregisteredCustomerMessage =
          "هذا العميل غير مسجل في النظام. قد تفقد بعض المميزات.";
      }
    },
    updateTechnicalId() {
      ...
      if (selectedTechnicalObj) {
        ...
        this.unregisteredTechnicalMessage =
          "هذا الفني غير مسجل في النظام. قد تفقد بعض المميزات.";
      }
    },
      ...
      try {
        ...
        if (this.customInvoice) {
          sweetalert(
            ...
            "سيتم إعادة توجيهك إلى صفحة تخصيص فاتورة.",
            "success"
          );
          this.$router.push({
            name: "CustomInvoice",
            params: { orderId: this.Id, isCustom: "true" },
          });
        } else {
          sweetalert(
            "تم تعديل الطلب بنجاح!",
          ...
          this.$router.push({
            name: "Fatora",
            params: { orderId: this.Id, isCustom: "false" },
          });
        }
      } catch (error) {
