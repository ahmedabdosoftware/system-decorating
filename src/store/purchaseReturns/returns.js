...
export const useReturnsStore = defineStore("useReturnsStore", {
  ...
  actions: {
    ...
    async fetchReturnsByPurchaseId(purchaseId) {
      ...
      this.returns = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      return this.returns;
    ...
    async checkWarehouseStock(branchId, productsToReturn) {
      for (const product of productsToReturn) {
        ...
        const warehouseProduct = warehouseData.products.find(
          (p) => p.id === id
        );
    ...
    async addReturn(purchaseId, returnData) {
      ...
      const validation = await this.canReturnProducts(
        purchaseId,
        returnData.products
      );
      ...
      const stockValidation = await this.checkWarehouseStock(
        BranchId,
        returnData.products
      );
