  ...
  actions: {
    ...
    async fetchTransactionsByUserId(userId) {
      const querySnapshot = await db
        ...
        .where("userId", "==", userId)
        .get();
      this.userTransactions = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
    },
    ...
    async fetchSpecificTransactions() {
      const querySnapshot = await db
        .collection("SpecificTransactions")
        .where("status", "==", "open")
        .get();
      this.specificTransactions = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
    },
    ...
    async fetchSpecificTransactionByUserId(userId) {
      const querySnapshot = await db
        ...
        .where("userId", "==", userId)
        .get();
      this.specificUserTransactions = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
    },
          ...
          users.map(async (user) => {
            ...
            const userTransactions = querySnapshot.docs.map((doc) => {
              ...
              if (bothType && bothType.totalAmount) {
                ...
                const materials = transaction.typesData.find(
                  (t) => t.type === "materials"
                );
                const manufacturing = transaction.typesData.find(
                  (t) => t.type === "manufacturing"
                );

                const materialsRemaining = materials
                  ? Number(materials.remainingValue) || 0
                  : 0;
                const manufacturingRemaining = manufacturing
                  ? Number(manufacturing.remainingValue) || 0
                  : 0;
  ...
  getters: {
    totalRemainingBalance() {
      return this.userTransactionSummaries.reduce((total, user) => {
        const userTotal = user.transactions.reduce(
          (sum, transaction) => sum + Number(transaction.remainingBalance),
          0
        );
