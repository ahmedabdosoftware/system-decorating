  ...
  actions: {
    ...
    async fetchTransactionsByProfile(profileId) {
      try {
        ...
        const totalPull = pullTransactions.reduce(
          (sum, t) => sum + t.amount,
          0
        );
      ...
      try {
        ...
        const transactionsArray = await Promise.all(
          profileIds.map(async (profileId) => {
            ...
            const totalAdd = addTransactions.reduce(
              (sum, t) => sum + t.amount,
              0
            );
            const totalPull = pullTransactions.reduce(
    ...
    async deleteTransaction(id, typeTransaction, profileId) {
      ...
      try {
        if (typeTransaction == "Add") {
          ...
          this.addTransactions = this.addTransactions.filter(
            (transaction) => transaction.id !== id
          );
          this.fetchTransactionsByProfile(profileId);
        } else {
          await db.collection("PullRandomTransaction").doc(id).delete();
          this.pullTransactions = this.pullTransactions.filter(
            (transaction) => transaction.id !== id
          );
          this.fetchTransactionsByProfile(profileId);
        }
